...
def get_scores_alpha_matte(ground_truth_dir, predicted_dir, save_path, kernel_size=4, power=1):
    ...
    scores = pd.DataFrame(columns=['mse','sad','mad','norm'])
    for i, img_name in enumerate(ground_truth_paths):
        ...
        mse = calc_mse(gt, pred)
        sad = calc_sad(gt, pred)
        mad = calc_mad(gt, pred)
        norm = calc_error_normalized(gt, pred)
        scores.loc[img_name] = [mse, sad, mad, norm]
        if i % 200 == 0:
...
save_path = r'C:\Users\simon\PycharmProjects\image_background_removal\data\scores.csv'
get_scores_alpha_matte(gt_dir, pred_dir, save_path)
